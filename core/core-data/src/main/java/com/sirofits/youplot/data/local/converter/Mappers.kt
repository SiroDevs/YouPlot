package com.sirofits.youplot.data.local.converter
...
fun List<Waypoint>.toJson(): String {
    ...
    forEach { wp ->
        arr.put(JSONObject().apply {
            put("id", wp.id)
            put("routeId", wp.routeId)
            put("name", wp.name)
            put("lat", wp.position.latitude)
            put("lng", wp.position.longitude)
            put("orderIndex", wp.orderIndex)
            put("elevation", wp.elevationMeters)
...
fun List<WaypointProgress>.toJson(): String {
    ...
    forEach { wp ->
        arr.put(JSONObject().apply {
            ...
            put("planned", wp.plannedArrivalMillis)
            put("estimated", wp.estimatedArrivalMillis)
            put("remaining", wp.distanceRemainingKm)
            put("reached", wp.isReached)
...
fun Route.toEntity() = RouteEntity(
    ...
    waypointsJson = waypoints.toJson(),
    elevationProfileJson = elevationProfile.toJson(),
...
fun RouteEntity.toDomain() = Route(
    ...
    waypoints = waypointsJson.toWaypointList(),
    elevationProfile = elevationProfileJson.toElevationList(),
    totalDistanceKm = totalDistanceKm,
    totalElevationGainMeters = totalElevationGainMeters,
    totalElevationLossMeters = totalElevationLossMeters,
    isRoundTrip = isRoundTrip,
...
fun ActivityPlan.toEntity() = PlanEntity(
    ...
    avgDistancePerDayKm = avgDistancePerDayKm,
    createdAt = createdAt,
)
...
fun SessionEntity.toDomain() = ActivitySession(
    ...
    planId = planId,
    routeId = routeId,
    status = com.sirofits.youplot.domain.entity.SessionStatus.valueOf(status),
    startedAtMillis = startedAtMillis,
    elapsedTimeSeconds = elapsedTimeSeconds,
