package com.sirofits.youplot.domain.usecase.plan
...
class GeneratePlanEventsUseCase @Inject constructor(
    ...
    suspend operator fun invoke(plan: ActivityPlan): List<PlanEvent> {
        ...
        for (day in 1..plan.numberOfDays) {
            ...
            // Auto-place waypoints that fall within this day's distance window
            val dayEndDistance = cumulativeDistance + distancePerDay
            route.waypoints
                .filter { it.orderIndex > 0 } // skip start
                .forEach { waypoint ->
            ...
            dayStart = dayEndMillis + 10 * 3_600_000L // next day start ~10h later (sleep)
        }
        return events
    }
}
