...
class _MapPointPickerState extends State<MapPointPicker> {
  ...
  void _initCamera() {
    ...
    // Try to use the session origin as the camera anchor. Fall back to the
    // user's GPS location from LocationSearchBloc if the session has no origin
    // (e.g. when the picker is opened before an origin is set).
    final sessionState = context.read<RouteSessionCubit>().state;
    final lsState = context.read<LocationSearchBloc>().state;

    final anchor = sessionState.origin ??
                  ...
                  child: BlocConsumer<LocationSearchBloc, LocationSearchState>(
                    ...
                    listener: (_, s) {
                      if (s.currentLocation != null) {
                        _mapController.move(
                          LatLng(s.currentLocation!.lat, s.currentLocation!.lng),
                          14,
                        );
                      }
                    },
