package com.sirofits.youplot.plan.viewmodel
...
import com.sirofits.youplot.domain.usecase.plan.*
import com.sirofits.youplot.domain.usecase.route.GetAllRoutesUseCase
import com.sirofits.youplot.domain.usecase.route.GetRouteByIdUseCase
...
data class PlanCreatorUiState(
    ...
    val currentStep: Int = 0,         // 0=route, 1=setup, 2=events, 3=review
...
class PlanCreatorViewModel @Inject constructor(
    ...
    fun selectRoute(route: Route) {
        _state.update {
            it.copy(
                selectedRoute = route,
                avgDistancePerDayKm = route.totalDistanceKm,
    ...
    fun setNumberOfDays(days: Int) = _state.update {
        it.copy(
            numberOfDays = days.coerceAtLeast(1),
        ...
        when (s.currentStep) {
            0 -> {
                if (s.selectedRoute == null) {
                    _state.update { it.copy(error = "Please select a route first") }
                    return
                }
                _state.update { it.copy(currentStep = 1) }
            }
            1 -> {
                // Auto-generate events before moving to review
                viewModelScope.launch { generateEvents() }
            }
    ...
    private suspend fun generateEvents() {
        ...
        val draft = ActivityPlan(
            ...
            startDateMillis = s.startDateMillis,
    ...
    fun addCustomEvent(event: PlanEvent) = _state.update {
        it.copy(customEvents = it.customEvents + event)
    }
